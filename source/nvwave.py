...
class WavePlayer(object):
	...
	def feed(self, data):
		...
		"""
		if self._audioDucker: self._audioDucker.requestDucking(True)
		whdr = WAVEHDR()
		whdr.lpData = data
	...
	def pause(self, switch):
		...
		@type switch: bool
		"""
		if self._audioDucker: self._audioDucker.requestDucking(not switch)
		with self._waveout_lock:
		...
		with self._lock:
			...
			with self._waveout_lock:
				...
				if self.closeWhenIdle:
					self._close()
		if self._audioDucker: self._audioDucker.requestDucking(False)
